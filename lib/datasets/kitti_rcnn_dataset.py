...
class KittiRCNNDataset(KittiDataset):
    def __init__(self, root_dir, npoints = 16384, split = 'train', classes = 'Car', mode = 'TRAIN',
                 ...
                 save_dir=None, blackbox_attack=False, choice_dir=None):
        super().__init__(root_dir=root_dir, split=split, blackbox_attack=blackbox_attack)
        if classes == 'Car':
    ...
    def get_rpn_with_li_fusion(self, index):
        ...
        if self.mode == 'TRAIN' or self.random_select:
            ...
            if self.choice_dir is not None:
                ...
                if self.npoints < len(pts_rect):
                    ...
                    np.random.shuffle(choice)
                if self.save_dir is not None and self.mode == 'EVAL':
                    choice_file = os.path.join(self.save_dir, '%06d.npy' % sample_id)
