...
class PointRCNN(nn.Module):
    ...
    def forward(self, input_data, attack=False):

        if cfg.RPN.ENABLED:
            output = { }
            # rpn inference
            if self.training or attack:
                if cfg.RPN.FIXED:
                    self.rpn.eval()
            ...
            if cfg.RCNN.ENABLED:
                with torch.no_grad():
                    ...
                    # proposal layer
                    rois, roi_scores_raw = self.rpn.proposal_layer(rpn_scores_raw, rpn_reg,
                                                                   backbone_xyz)  # (B, M, 7)
                ...
                rcnn_output = self.rcnn_net(rcnn_input_info, attack=attack)
                output.update(rcnn_output)

        elif cfg.RCNN.ENABLED:
            output = self.rcnn_net(input_data, attack=attack)
        else:
            raise NotImplementedError
