...
from lib.net.GAN_model import Generator_img, define_Dimg, Perturbation
...
parser.add_argument('--lr_ppts', type=float, default=0.01)
...
if __name__ == "__main__":
    ...
    fn_decorator = train_functions.model_afus_fn_decorator()
    ...
    if args.mgpus:
        discriminator = define_Dimg(gpu_ids=range(torch.cuda.device_count()))
        model = nn.DataParallel(model)
        generator = nn.DataParallel(generator)
        pts_pert = nn.DataParallel(pts_pert)
    else:
        discriminator = define_Dimg(input_nc=3, ndf=64)
        discriminator.cuda()
    model.cuda()
    generator.cuda()
    pts_pert.cuda()

    optimizer_Gimg = optim.Adam(generator.parameters(), lr=args.lr)
    optimizer_Dimg = optim.Adam(discriminator.parameters(), lr=args.lr*2, betas=(0.5, 0.999))
    ...
    trainer = train_utils.Trainer_Afus(
            model,
            generator,
            discriminator,
            pts_pert,
