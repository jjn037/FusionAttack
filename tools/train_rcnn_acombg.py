...
from lib.net.GAN_model import Generator_img, define_Dimg, Generator_pts
...
parser.add_argument('--cg', type=float, default=0.05)
parser.add_argument('--c_dpts', type=float, default=0.05)
parser.add_argument('--std_ppts', type=float, default=0.0000001)
parser.add_argument('--lr_img', type=float, default=0.0001)
parser.add_argument('--lr_pts', type=float, default=0.001)
...
if __name__ == "__main__":
    ...
    fn_decorator = train_functions.model_acombg_fn_decorator()
    fn_eval_decorator = train_functions.model_aimg_fn_eval_decorator()

    model = PointRCNN(num_classes=train_loader.dataset.num_class, use_xyz=True, mode='TRAIN')
    generator_img = Generator_img(num_channels=3, ngf=100)
    # pts_pert = Perturbation(batch_size=args.batch_size, std=args.std_ppts)
    ...
    if args.mgpus:
        discriminator_img = define_Dimg(input_nc=3, ndf=64, gpu_ids=range(torch.cuda.device_count()))
        model = nn.DataParallel(model)
        generator_img = nn.DataParallel(generator_img)
        # pts_pert = nn.DataParallel(pts_pert)
        generator_pts = nn.DataParallel(generator_pts)
    else:
        discriminator_img = define_Dimg(input_nc=3, ndf=64)
        discriminator_img.cuda()
    model.cuda()
    generator_img.cuda()
    # pts_pert.cuda()
    generator_pts.cuda()

    optimizer_Gimg = optim.Adam(generator_img.parameters(), lr=args.lr_img)
    optimizer_Dimg = optim.Adam(discriminator_img.parameters(), lr=args.lr_img*2, betas=(0.5, 0.999))
    # optimizer_Ppts = optim.Adam(pts_pert.parameters(), lr=args.lr_ppts)
    optimizer_Gpts = optim.Adam(generator_pts.parameters(), lr=args.lr_pts)
    ...
    trainer = train_utils.Trainer_Afusg(
            model,
            generator_img,
            discriminator_img,
            generator_pts,
            ...
            model_fn_eval = fn_eval_decorator,
            tb_log = tb_log,
            eval_frequency = 1,
