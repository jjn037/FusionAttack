...
from lib.net.GAN_model import Perturbation
...
def eval_one_epoch_joint(model, pts_pert, dataloader, epoch_id, result_dir, logger):
    np.random.seed(666)
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    mode = 'TEST' if args.test else 'EVAL'
    ...
    for data in dataloader:
        ...
        input_data = { 'pts_input': inputs }
        if args.noise:
            input_data['pts_input'] = input_data['pts_input'] + args.pert
        else:
    ...
    if args.noise:
        avg_ppts_dist = torch.mean(reduce_sum(args.pert ** 2))
    else:
...
def eval_one_epoch(model, pts_pert, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ...
        ret_dict = eval_one_epoch_joint(model, pts_pert, dataloader, epoch_id, result_dir, logger)
    else:
...
def load_ckpt_based_on_args(model, pts_pert, logger):
...
def eval_single_ckpt(root_result_dir):
    ...
    # load checkpoint
    load_ckpt_based_on_args(model, pts_pert, logger)

    # start evaluation
    eval_one_epoch(model, pts_pert, test_loader, epoch_id, root_result_dir, logger)
...
if __name__ == "__main__":
    ...
    os.makedirs(root_result_dir, exist_ok = True)
    if args.noise:
        args.pert = torch.FloatTensor(1, 16384, 3)
        size = args.pert.shape
        tmp = args.pert.new_empty(size + (4,)).normal_()
        valid = (tmp < 2) & (tmp > -2)
        ind = valid.max(-1, keepdim=True)[1]
        args.pert.data.copy_(tmp.gather(-1, ind).squeeze(-1))
        args.pert.data.mul_(0.02).add_(0)
        args.pert = args.pert.cuda()
