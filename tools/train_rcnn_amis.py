...
from lib.net.GAN_model import Generator, define_D, Perturbation
...
if __name__ == "__main__":
    ...
    fn_decorator = train_functions.model_amis_fn_decorator()
    fn_eval_decorator = train_functions.model_aimg_fn_eval_decorator()

    model = PointRCNN(num_classes=train_loader.dataset.num_class, use_xyz=True, mode='TRAIN')
    generator = Generator(num_channels=3, ngf=100)
    pts_pert = Perturbation(batch_size=args.batch_size, std=args.std_ppts)

    # optimizer = create_optimizer(model)

    if args.mgpus:
        discriminator = define_D(gpu_ids=range(torch.cuda.device_count()))
        model = nn.DataParallel(model)
        generator = nn.DataParallel(generator)
        pts_pert = nn.DataParallel(pts_pert)
    else:
        discriminator = define_D(input_nc=3, ndf=64)
        discriminator.cuda()
    model.cuda()
    generator.cuda()
    pts_pert.cuda()
