...
from lib.net.GAN_model import Generator_img, Perturbation
...
def eval_one_epoch_joint(model, generator, pts_pert, dataloader, epoch_id, result_dir, logger):
    np.random.seed(666)
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    mode = 'TEST' if args.test else 'EVAL'
    ...
    for data in dataloader:
        ...
        # print(inputs.shape)
        # input('Pause:')
        input_data['pts_input'] = pts_pert(input_data['pts_input'])
        # img feature
        if cfg.LI_FUSION.ENABLED:
            pts_origin_xy, img = data['pts_origin_xy'], data['img']
            pts_origin_xy = torch.from_numpy(pts_origin_xy).cuda(non_blocking = True).float()
            ...
            input_data['pts_origin_xy'] = pts_origin_xy
            # input_data['img'] = img
            img_pert = generator(img)
            cur_dist = torch.mean(reduce_sum(img_pert ** 2))
            pert_dist += cur_dist
    ...
    avg_pimg_dist = (pert_dist / max(cnt, 1.0))
    avg_refined_dist = (refined_dist / max(cnt, 1.0))
    avg_ppts_dist = pts_pert.get_norm()
    logger.info('final average detections: %.3f' % avg_det_num)
...
def eval_one_epoch(model, generator, pts_pert, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ...
        ret_dict = eval_one_epoch_joint(model, generator, pts_pert, dataloader, epoch_id, result_dir, logger)
    else:
...
def load_ckpt_based_on_args(model, generator, pts_pert, logger):
    ...
    if args.afus_ckpt_dir is not None:
        ...
        pts_pert.load_state_dict(checkpoint['model_state'])
        logger.info("==> Done")
...
def eval_single_ckpt(root_result_dir):
    ...
    pts_pert = Perturbation(batch_size=2)
    ...
    # load checkpoint
    load_ckpt_based_on_args(model, generator, pts_pert, logger)

    # start evaluation
    eval_one_epoch(model, generator, pts_pert, test_loader, epoch_id, root_result_dir, logger)
