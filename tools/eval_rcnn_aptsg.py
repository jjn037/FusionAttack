...
from lib.net.GAN_model import Generator_pts, Generator_fusimg, Generator_fuspts
...
def eval_one_epoch_joint(model, generator_pts, dataloader, epoch_id, result_dir, logger, generator_img=None):
    np.random.seed(666)
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    mode = 'TEST' if args.test else 'EVAL'
...
def eval_one_epoch(model, generator_pts, dataloader, epoch_id, result_dir, logger, generator_img=None):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ...
        ret_dict = eval_one_epoch_joint(model, generator_pts, dataloader, epoch_id, result_dir, logger,
                                        generator_img=generator_img)
    else:
...
def load_ckpt_based_on_args(model, generator_pts, logger, generator_img=None):
...
def eval_single_ckpt(root_result_dir):
    ...
    # load checkpoint
    load_ckpt_based_on_args(model, generator_pts, logger, generator_img=generator_img)

    # start evaluation
    eval_one_epoch(model, generator_pts, test_loader, epoch_id, root_result_dir, logger, generator_img=generator_img)
