...
def eval_one_epoch_joint(model, generator, dataloader, epoch_id, result_dir, logger):
    np.random.seed(666)
    MEAN_SIZE = torch.from_numpy(cfg.CLS_MEAN_SIZE[0]).cuda()
    mode = 'TEST' if args.test else 'EVAL'
    ...
    for data in dataloader:
        ...
        if cfg.LI_FUSION.ENABLED:
            ...
            if args.noise:
                img = img + args.pert
                input_data['img'] = torch.from_numpy(img).cuda(non_blocking = True).float().permute((0,3,1,2))
            else:
                img = torch.from_numpy(img).cuda(non_blocking=True).float().permute((0, 3, 1, 2))
                # input_data['img'] = img
                if args.fusion:
                    perturbation, _ = generator(img)
                else:
                    perturbation = generator(img)
                cur_dist = torch.mean(reduce_sum(perturbation ** 2))
                pert_dist += cur_dist
                input_data['img'] = img + perturbation
                for j in range(3):
                    input_data['img'][:, j, :, :] = torch.clamp(input_data['img'][:, j, :, :],
                                                                min=clamp_min[j], max=clamp_max[j])
                cur_dist_r = torch.mean(reduce_sum((input_data['img'] - img) ** 2))
                refined_dist += cur_dist_r
        ...
        if args.noise:
            ...
            disp_dict = {'mode': mode, 'recall': '%d/%d' % (total_recalled_bbox_list[3], total_gt_bbox),
                         'pert_dist': cur_dist.item(), 'refined_dist': cur_dist_r.item()}
        progress_bar.set_postfix(disp_dict)
        progress_bar.update()
    ...
    if args.noise:
        avg_pert_dist = np.sum(args.pert ** 2)
        avg_refined_dist = avg_pert_dist
    else:
        avg_pert_dist = (pert_dist / max(cnt, 1.0))
        avg_refined_dist = (refined_dist / max(cnt, 1.0))
    logger.info('final average detections: %.3f' % avg_det_num)
    logger.info('final average rpn_iou refined: %.3f' % avg_rpn_iou)
    logger.info('final average cls acc: %.3f' % avg_cls_acc)
    logger.info('final average cls acc refined: %.3f' % avg_cls_acc_refined)
    logger.info('final average pert dist: %.3f' % avg_pert_dist)
    logger.info('final average pert dist refined: %.3f' % avg_refined_dist)
    ret_dict['rpn_iou'] = avg_rpn_iou
    ret_dict['rcnn_cls_acc'] = avg_cls_acc
    ret_dict['rcnn_cls_acc_refined'] = avg_cls_acc_refined
    ret_dict['rcnn_avg_num'] = avg_det_num
    ret_dict['pert_dist'] = avg_pert_dist
    ret_dict['pert_dist_refined'] = avg_refined_dist
...
def eval_one_epoch(model, generator, dataloader, epoch_id, result_dir, logger):
    if cfg.RPN.ENABLED and not cfg.RCNN.ENABLED:
        ...
        ret_dict = eval_one_epoch_joint(model, generator, dataloader, epoch_id, result_dir, logger)
    else:
...
def load_ckpt_based_on_args(model, generator, logger):
...
def eval_single_ckpt(root_result_dir):
    ...
    if args.model_type == 'base':
        model = PointRCNN(num_classes = test_loader.dataset.num_class, use_xyz = True, mode = 'TEST')
    if args.fusion:
        generator = Generator_fusimg(num_channels=3, ngf=100)
    else:
        generator = Generator_img(num_channels=3, ngf=100)
    ...
    # load checkpoint
    load_ckpt_based_on_args(model, generator, logger)

    # start evaluation
    eval_one_epoch(model, generator, test_loader, epoch_id, root_result_dir, logger)
...
if __name__ == "__main__":
    ...
    os.makedirs(root_result_dir, exist_ok = True)
    if args.noise:
        args.pert = np.random.normal(0.0, 0.1, size=(1, 384, 1280, 3))
        pert_file = os.path.join(root_result_dir, 'pert.npy')
